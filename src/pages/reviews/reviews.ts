...
export class ReviewPage {
    ...
    constructor(public translate: TranslateService, public nav: NavController, public service: ProductService, params: NavParams, public functions: Functions, public values: Values, public toastCtrl: ToastController) {
  ...
  ngOnInit() {
        this.translate.get(['ErrorTitle', 'Success', 'ReviewMesssage', 'PleaseEnterEmailID', 'PleaseEnterName', 'PleaseEnterComment']).subscribe(translations => {
            this.lan.Success = translations.Success;
            this.lan.ErrorTitle = translations.ErrorTitle;
            this.lan.ReviewMesssage = translations.ReviewMesssage;
            this.lan.PleaseEnterEmailID = translations.PleaseEnterEmailID;
            this.lan.PleaseEnterName = translations.PleaseEnterName;
            this.lan.PleaseEnterComment = translations.PleaseEnterComment;
        })
    }
  ...
  submitComment() {
      ...
      if (this.validate()) {
          ...
          this.service.submitComment(this.form).then((results) => {
              ...
              this.functions.showAlert(this.lan.Success, this.lan.ReviewMesssage);
          });
      }
  }
  validate() {
      if (!this.values.isLoggedIn) {
          if (this.form.author == undefined || this.form.author == "") {
              this.functions.showAlert(this.lan.ErrorTitle, this.lan.PleaseEnterName);
              return false
          }
      ...
      if (this.form.comment == undefined || this.form.comment == "") {
          this.functions.showAlert(this.lan.ErrorTitle, this.lan.PleaseEnterComment);
          return false
      } else return true;
  }
  showSubmitReview() {
