...
export class CheckoutService {
    ...
    submitCoupon(form) {
        ...
        return new Promise(resolve => {
            this.http.post(this.config.url + '/wp-admin/admin-ajax.php?action=mstoreapp-apply_coupon', params).subscribe(data => {
                this.status = data;
                resolve(this.status);
            });
        });
    ...
    getStripeToken(form) {
        ...
        return new Promise(resolve => {
            this.http.post('https://api.stripe.com/v1/tokens', params, this.config.options).map(res => res.json()).subscribe(data => {
                this.status = data;
                resolve(this.status);
    ...
    getOrderSummary(id) {
        return new Promise(resolve => {
            this.http.get(this.config.setUrl('GET', '/wp-json/wc/v2/orders/' + id + '?', false), this.config.optionstwo).map(res => res.json()).subscribe(data => {
                this.http.get(this.config.url + '/wp-admin/admin-ajax.php?action=mstoreapp-emptyCart', this.config.options).map(res => res.json()).subscribe(data => {
                });
